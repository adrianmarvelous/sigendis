		...
		if(isset($_COOKIE['adv_token'])){
			...
			if($cek){
				...
				if($row['ip'] == $_SERVER['REMOTE_ADDR'] || $row['useragent'] == $_SERVER['HTTP_USER_AGENT']){
					...
					$query_user = $this->db->query("SELECT * FROM user_gedung_diklat WHERE username = ".$this->db->quote($username));
					$data_query_user = $query_user->fetch();
					...
					if($query_user->rowCount() != 0){
						return array(
							$_SESSION['username'] = $data_query_user['username'],
							$_SESSION['role'] = $data_query_user['role']
						);
					}
				}
